define(function (require) {
	...
	var overrides = _.map({
			...
			response.prettyType = response.type.replace(/-?(\w+)/gi, function (_, word) {
			...
			response.url = (response.url) ? response.url : [
					'/entity/', response.type, '/', response.uuid
			].join('');
	...
	return function applyAccountModelDefaults(account, model) {
